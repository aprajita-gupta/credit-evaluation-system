...
def compute_shap_values(model, model_name, X_train, X_test, feature_names, skip_slow=False):
    """
    Compute SHAP values for model interpretability
    Returns: shap_values, expected_value, explainer
    
    Args:
        skip_slow: If True, skip SVM (which is very slow with KernelExplainer)
    """
    if not SHAP_AVAILABLE:
        return None, None, None
    
    # Skip SVM if requested (it's very slow with KernelExplainer)
    if skip_slow and model_name == 'SVM':
        print(f"    Skipping SHAP for {model_name} (slow computation)")
        return None, None, None
    ...
    try:
        ...
        if model_name in ['Decision Tree', 'Random Forest']:
            ...
            # Use very small samples for SVM to avoid excessive computation time
            background = shap.sample(X_train, 25)  # Reduced from 50
            explainer = shap.KernelExplainer(model.predict_proba, background)
            shap_values = explainer.shap_values(X_test[:50])  # Limit to 50 samples
...
def create_shap_visualizations(results, feature_names, save_dir='static'):
    ...
    for model_name in model_names:
        ...
        try:
            ...
            try:
                # Use newer SHAP API (v0.20+)
                plt.figure(figsize=(20, 3))
                shap.plots.force(expected_value, shap_values[0], 
                               X_test_shap[0], 
                               feature_names=feature_names,
            ...
            if len(feature_names) > 0:
                ...
                try:
                    ...
                    top_feature_idx = np.argmin(mean_abs_shap)
                    
                    # Ensure index is within bounds
                    if top_feature_idx >= X_test_shap.shape[1]:
                        top_feature_idx = 0
    ...
    for name, model in models.items():
        ...
        if compute_shap and SHAP_AVAILABLE:
            shap_values, expected_value, explainer = compute_shap_values(
                model, name, X_train, X_test, feature_names, skip_slow=True  # Skip SVM by default
            )
    ...
    if TENSORFLOW_AVAILABLE:
        ...
        if compute_shap and SHAP_AVAILABLE:
            shap_values_dnn, expected_value_dnn, explainer_dnn = compute_shap_values(
                dnn_model, 'Deep Neural Network', X_train, X_test, feature_names, skip_slow=False
            )
        
        results['Deep Neural Network'] = {
            'model': dnn_model,
            'training_time': train_time_dnn,
            'cv_accuracy_mean': 0,
            'cv_accuracy_std': 0,
            'cv_f1_mean': 0,
            'cv_f1_std': 0,
...
def save_results_csv(results, save_dir='static'):
    ...
    for model in model_names:
        ...
        data.append({
            ...
            'F1_Score': f"{r['f1']:.4f}",
            'Training_Time_s': f"{r['training_time']:.4f}",
